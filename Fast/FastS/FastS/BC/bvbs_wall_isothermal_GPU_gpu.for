...
c     $Author: IvanMary $
c***********************************************************************
      subroutine bvbs_wall_isothermal_GPU(idir,lrhs, nstep,
     &                         neq_mtr, mobile_coef,
...
#include     "FastS/BC/BCWallViscousIsothermal_i.for"
             do i = ind_loop(1), ind_loop(2)-1
               l    = inddm( i              , j,  k ) 
               ldp  = indven( ind_loop(2)+1 , j,  k )  !next rank
#include       "FastS/BC/BCWallViscousIsothermal_i.for"               !next rank
             enddo
          enddo
          enddo
          ...
          do j = ind_loop(3), ind_loop(4)
             ...
             l    = inddm( i              , j,  k ) 
             ldp  = indven( ind_loop(2)+1 , j,  k )
#include     "FastS/BC/BCWallViscousIsothermalSA_i.for"
             do i = ind_loop(1), ind_loop(2)-1
               l    = inddm( i              , j,  k ) 
               ldp  = indven( ind_loop(2)+1 , j,  k )  !next rank
#include       "FastS/BC/BCWallViscousIsothermalSA_i.for"             !next rank
             enddo
          enddo
          enddo
...
#include        "FastS/BC/BCWallViscousIsothermal_j.for"
              enddo 
...
#include        "FastS/BC/BCWallViscousIsothermal_j.for"              !next rank
              enddo 
            enddo !j
          enddo !k
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                l    = inddm(  i,          j    , k ) 
                ldp  = indven( i, ind_loop(4)+1 , k )
#include        "FastS/BC/BCWallViscousIsothermalSA_j.for"
              enddo 
            do j = ind_loop(3), ind_loop(4)-1
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                l    = inddm(  i,          j    , k ) 
                ldp  = indven( i, ind_loop(4)+1 , k )  !next rank
#include        "FastS/BC/BCWallViscousIsothermalSA_j.for"            !next rank
              enddo 
            enddo !j
          enddo !k
...
#include        "FastS/BC/BCWallViscousIsothermal_k.for"
              enddo 
           enddo 
...
#include        "FastS/BC/BCWallViscousIsothermal_k.for"                   !next rank
              enddo 
            enddo !j
          enddo !k
...
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                l    = inddm(  i,      j, k              ) 
                ldp  = indven( i,      j, ind_loop(6)+1  )
#include        "FastS/BC/BCWallViscousIsothermalSA_k.for"
              enddo 
           enddo 
          do k = ind_loop(5), ind_loop(6)-1
            do j = ind_loop(3), ind_loop(4)
!DEC$ IVDEP
              do i = ind_loop(1), ind_loop(2)
                l    = inddm(  i,      j, k              ) 
                ldp  = indven( i,      j, ind_loop(6)+1  )  !next rank
#include        "FastS/BC/BCWallViscousIsothermalSA_k.for"                 !next rank
              enddo 
            enddo !j
          enddo !k
       endif !param_int(NEQ)
