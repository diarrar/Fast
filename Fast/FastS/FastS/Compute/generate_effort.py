...
select= open('template_effort_select.for','r')
...
fselecto = open(rep+'/effort_'+flux+'_select.for',"w")                  # ouvrir le fichier de sortie
            ...
            for typezone in TypeMesh:
                ...
                if typezone == '2d':
                    ...
                    for i in range( len(lines) ):
                        ...
                        lines[i]=lines[i].replace("flu(lf+v2flu*8","flu(lf+v2flu*7").replace("flu(lf+v2flu*9","flu(lf+v2flu*8")
                ...
                if slope != 'minmod':
                    ...
                    for l in lines:
                        ...
                        c += 1
                    for i in range( len(lines) ):
                        lines[i]=lines[i].replace("psiroe,avmin, ", 'psiroe, ').replace("psiroe,avmin,", 'psiroe,').replace("psiroe,avmin", 'psiroe')
                ...
                if include:
                    ...
                    select_out.append('     &                 param_int, param_real,param_int_eff,\n')
                    select_out.append('     &                 ind_loop, effort, pos,\n')
                    select_out.append('     &                 rop, flu , wig,\n')
                    select_out.append('     &                 x, y, z,\n')
...
for i in range( len(lines_select_beg) ):
    lines_select_beg[i]=lines_select_beg[i].replace("template_effort_select"                ,'eff_'+flux+'_select' )
