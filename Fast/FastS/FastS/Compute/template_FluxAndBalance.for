C ** Base Template: Master template for CPU flux computations
C ** Contains conditional GPU offloading directives for unified codebase

C***********************************************************************
      subroutine flu_lam_template(ndom, ithread,
...
CC!DIR$ ASSUME_ALIGNED xmut: CACHELINE
      ...
      v1ven =   0                                       !ALE only
      v2ven =   param_int(NDIMDX_VENT)                  !ALE only
      v3ven = 2*param_int(NDIMDX_VENT)*ck_vent          !ALE only
...
#include    "FastS/Compute/assemble_drodm_plus_vec1.for"  !3D only
          enddo                             !3D only
...
!$OMP END TARGET DATA
                                                                       ...
                                                                       !3D only
                                                                       !3D only
      !Complement fluk en Kmax                                         !3D only
      If(kcache.eq.ijkv_cache(3).and.synchro_receive_th(3).eq.0) then  !3D only
                                                                       !3D only
        k    = ind_loop(6)+1               !3D only
        do j = ind_loop(3),ind_loop(4)     !3D only
                                           !3D only
#include "FastS/Compute/loopI_begin.for"                 !3D only
                                           !3D only
#include   "FastS/Compute/FLUX_CONV/fluFaceEuler_k.for"  !3D only
...
#include   "FastS/Compute/fluViscRans_k.for"             !3D only
            ls = l -inck                   !3D only
#include    "FastS/Compute/flu_send_scater_vec1.for"     !3D only
          enddo                            !3D only
        enddo                              !3D only
      Endif                                !3D only
      end
