...
def miseAPlatDonorTree__(t, tc, graph=None, procDict=None, graphID_Unsteady=None, nbpts_linelets=0, FilterPass=None):
    ...
    #print('graph pass', FilterPass,'=',  graphID, flush=True)
    ...
    for base in bases:
        ...
        for z in zones_tc:
            ...
            #construction dico pour flux conservatif
            ...
            for s in subRegions:
                ...
                # recherche nombre raccord conservatif dans la pass
                zd = zones[c]
                nbfluCons = 0
                bcs = Internal.getNodesFromType2(zd, 'BC_t')
                bc_info = []
    ...
    if not graphliste: # Si le graph n est pas une liste, on n'est pas en explicite local
        ...
        graphIDrcv_=[]; pos_ID=[]; S_ID=len(graph)
    ...
    #print("len graphIDrcv  is",len(graphIDrcv), 'pass', FilterPass, flush=True)
    #print("pos_ID  is",pos_ID)


    #param_int  = numpy.empty(size_int + len(graphIDrcv) + len(graphIBCrcv) + 2, dtype=Internal.E_NpyInt)
    param_int  = numpy.empty(size_int + len(graphIDrcv) + 1, dtype=Internal.E_NpyInt)
...
#           print('IBCType = ', IBCType)
        ...
        if sname == 'IB':
            ...
            if xcInit is not None:
                xcInit[1] = param_real[ ptxcInit : ptxcInit + Nbpts_D ]
                ycInit[1] = param_real[ ptycInit : ptycInit + Nbpts_D ]
                zcInit[1] = param_real[ ptzcInit : ptzcInit + Nbpts_D ]

                xiInit[1] = param_real[ ptxiInit : ptxiInit + Nbpts_D ]
                yiInit[1] = param_real[ ptyiInit : ptyiInit + Nbpts_D ]
                ziInit[1] = param_real[ ptziInit : ptziInit + Nbpts_D ]

                xwInit[1] = param_real[ ptxwInit : ptxwInit + Nbpts_D ]
                ywInit[1] = param_real[ ptywInit : ptywInit + Nbpts_D ]
                zwInit[1] = param_real[ ptzwInit : ptzwInit + Nbpts_D ]

            if motion_type is not None:
                motion_type[1]  = param_real[ ptmotion_type : ptmotion_type + Nbpts_D ]

                transl_speedX[1] = param_real[ pttransl_speedX : pttransl_speedX + Nbpts_D ]
                transl_speedY[1] = param_real[ pttransl_speedY : pttransl_speedY + Nbpts_D ]
                transl_speedZ[1] = param_real[ pttransl_speedZ : pttransl_speedZ + Nbpts_D ]

                axis_pntX[1]     = param_real[ ptaxis_pntX : ptaxis_pntX + Nbpts_D ]
                axis_pntY[1]     = param_real[ ptaxis_pntY : ptaxis_pntY + Nbpts_D ]
                axis_pntZ[1]     = param_real[ ptaxis_pntZ : ptaxis_pntZ + Nbpts_D ]

                axis_vctX[1]     = param_real[ ptaxis_vctX : ptaxis_vctX + Nbpts_D ]
                axis_vctY[1]     = param_real[ ptaxis_vctY : ptaxis_vctY + Nbpts_D ]
                axis_vctZ[1]     = param_real[ ptaxis_vctZ : ptaxis_vctZ + Nbpts_D ]

                omega[1]        = param_real[ ptomega : ptomega + Nbpts_D ]
    ...
    for ntype in Nbtype:
        ...
        for i in range(Nbpts_D):
            ...
            if meshtype == 1:
                ...
                elif abs(ltype) == 2 : sizecoef =  8
                elif abs(ltype) == 22: sizecoef =  4
                elif abs(ltype) == 44: sizecoef = 12
...
def triMonoType(Nbpts_D, Nbpts, Nbpts_InterpD, meshtype, noi, lst,lstD,l0,ctyp,ptTy,shift_typ,pt_coef,nocoef,sname,Nbtype,
    ...
    ideb =  ptTy + shift_typ
    val = float(ltype)
    fastc.initNumaTransfer(None, param_int, ideb, Nbpts_D, 1, val)

    # recopie pointlist
    ideb = lst
    fastc.initNumaTransfer(pointlist[1], param_int, ideb, Nbpts, 1, val)
    #recopie pointListDonor
    ideb = lstD
    fastc.initNumaTransfer(pointlistD[1], param_int, ideb, Nbpts_D, 1, val)
    #recopie Maillage IBC
    if sname == 'IB':
        fastc.initNumaTransfer(xc[1], param_real, ptxc, Nbpts_D , 0, val)
        fastc.initNumaTransfer(yc[1], param_real, ptyc, Nbpts_D , 0, val)
        fastc.initNumaTransfer(zc[1], param_real, ptzc, Nbpts_D , 0, val)
        fastc.initNumaTransfer(xi[1], param_real, ptxi, Nbpts_D , 0, val)
        fastc.initNumaTransfer(yi[1], param_real, ptyi, Nbpts_D , 0, val)
        fastc.initNumaTransfer(zi[1], param_real, ptzi, Nbpts_D , 0, val)
        fastc.initNumaTransfer(xw[1], param_real, ptxw, Nbpts_D , 0, val)
        fastc.initNumaTransfer(yw[1], param_real, ptyw, Nbpts_D , 0, val)
        fastc.initNumaTransfer(zw[1], param_real, ptzw, Nbpts_D , 0, val)

        fastc.initNumaTransfer(density[1], param_real, ptdensity , Nbpts_D , 0, val)
        fastc.initNumaTransfer(pressure[1], param_real, ptpressure, Nbpts_D , 0, val)

        fastc.initNumaTransfer(vx[1], param_real, ptvx, Nbpts_D , 0, val)
        fastc.initNumaTransfer(vy[1], param_real, ptvy, Nbpts_D , 0, val)
        fastc.initNumaTransfer(vz[1], param_real, ptvz, Nbpts_D , 0, val)

        if utau is not None:
            fastc.initNumaTransfer(utau[1], param_real, ptutau, Nbpts_D, 0, val)
            fastc.initNumaTransfer(yplus[1], param_real, ptyplus, Nbpts_D , 0, val)

        if temp_local is not None:
            fastc.initNumaTransfer(temp_local[1]      , param_real, pttemp_local       , Nbpts_D , 0, val)
        if temp_extra_local is not None:
            fastc.initNumaTransfer(temp_extra_local[1], param_real, pttemp_extra_local , Nbpts_D , 0, val)
        if temp_extra2_local is not None:
            fastc.initNumaTransfer(temp_extra2_local[1], param_real, pttemp_extra2_local , Nbpts_D , 0, val)

        if wmodel_local_dens is not None:
            fastc.initNumaTransfer(wmodel_local_dens[1]      , param_real, pt_dens_wm_local       , Nbpts_D , 0, val)
            fastc.initNumaTransfer(wmodel_local_velx[1]      , param_real, pt_velx_wm_local       , Nbpts_D , 0, val)
            fastc.initNumaTransfer(wmodel_local_vely[1]      , param_real, pt_vely_wm_local       , Nbpts_D , 0, val)
            fastc.initNumaTransfer(wmodel_local_velz[1]      , param_real, pt_velz_wm_local       , Nbpts_D , 0, val)
            fastc.initNumaTransfer(wmodel_local_temp[1]      , param_real, pt_temp_wm_local       , Nbpts_D , 0, val)
            fastc.initNumaTransfer(wmodel_local_sanu[1]      , param_real, pt_sanu_wm_local       , Nbpts_D , 0, val)


        if gradxP is not None:
            fastc.initNumaTransfer(gradxP[1] , param_real, ptgradxP , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradyP[1] , param_real, ptgradyP , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradzP[1] , param_real, ptgradzP , Nbpts_D , 0, val)

        if gradxU is not None:
            fastc.initNumaTransfer(gradxU[1] , param_real, ptgradxU , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradyU[1] , param_real, ptgradyU , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradzU[1] , param_real, ptgradzU , Nbpts_D , 0, val)

        if gradxV is not None:
            fastc.initNumaTransfer(gradxV[1] , param_real, ptgradxV , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradyV[1] , param_real, ptgradyV , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradzV[1] , param_real, ptgradzV , Nbpts_D , 0, val)

        if gradxW is not None:
            fastc.initNumaTransfer(gradxW[1] , param_real, ptgradxW , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradyW[1] , param_real, ptgradyW , Nbpts_D , 0, val)
            fastc.initNumaTransfer(gradzW[1] , param_real, ptgradzW , Nbpts_D , 0, val)

        if xcInit is not None:
            fastc.initNumaTransfer(xcInit[1] , param_real, ptxcInit , Nbpts_D , 0, val)
            fastc.initNumaTransfer(ycInit[1] , param_real, ptycInit , Nbpts_D , 0, val)
            fastc.initNumaTransfer(zcInit[1] , param_real, ptzcInit , Nbpts_D , 0, val)

            fastc.initNumaTransfer(xiInit[1] , param_real, ptxiInit , Nbpts_D , 0, val)
            fastc.initNumaTransfer(yiInit[1] , param_real, ptyiInit , Nbpts_D , 0, val)
            fastc.initNumaTransfer(ziInit[1] , param_real, ptziInit , Nbpts_D , 0, val)

            fastc.initNumaTransfer(xwInit[1] , param_real, ptxwInit , Nbpts_D , 0, val)
            fastc.initNumaTransfer(ywInit[1] , param_real, ptywInit , Nbpts_D , 0, val)
            fastc.initNumaTransfer(zwInit[1] , param_real, ptzwInit , Nbpts_D , 0, val)

        if motion_type is not None:
            fastc.initNumaTransfer(motion_type[1]  , param_real, ptmotion_type  , Nbpts_D , 0, val)

            fastc.initNumaTransfer(transl_speedX[1] , param_real, pttransl_speedX , Nbpts_D , 0, val)
            fastc.initNumaTransfer(transl_speedY[1] , param_real, pttransl_speedY , Nbpts_D , 0, val)
            fastc.initNumaTransfer(transl_speedZ[1] , param_real, pttransl_speedZ , Nbpts_D , 0, val)

            fastc.initNumaTransfer(axis_pntX[1]     , param_real, ptaxis_pntX     , Nbpts_D , 0, val)
            fastc.initNumaTransfer(axis_pntY[1]     , param_real, ptaxis_pntY     , Nbpts_D , 0, val)
            fastc.initNumaTransfer(axis_pntZ[1]     , param_real, ptaxis_pntZ     , Nbpts_D , 0, val)

            fastc.initNumaTransfer(axis_vctX[1]     , param_real, ptaxis_vctX     , Nbpts_D , 0, val)
            fastc.initNumaTransfer(axis_vctY[1]     , param_real, ptaxis_vctY     , Nbpts_D , 0, val)
            fastc.initNumaTransfer(axis_vctZ[1]     , param_real, ptaxis_vctZ     , Nbpts_D , 0, val)

            fastc.initNumaTransfer(omega[1]        , param_real, ptomega        , Nbpts_D , 0, val)

        if kcurv is not None:
            fastc.initNumaTransfer(kcurv[1] , param_real, ptkcurv , Nbpts_D , 0, val)

        if sd1 is not None:
            fastc.initNumaTransfer(sd1[1], param_real, ptd1 , Nbpts_D , 0, val)
        if sd2 is not None:
            fastc.initNumaTransfer(sd2[1], param_real, ptd2 , Nbpts_D , 0, val)
        if sd3 is not None:
            fastc.initNumaTransfer(sd3[1] , param_real, ptd3 , Nbpts_D , 0, val)
        if sd4 is not None:
            fastc.initNumaTransfer(sd4[1] , param_real, ptd4 , Nbpts_D , 0, val)
        if sd5 is not None:
            fastc.initNumaTransfer(sd5[1] , param_real, ptd5 , Nbpts_D , 0, val)

        if yline is not None:
            fastc.initNumaTransfer(yline[1]          , param_real, ptyline, Nbpts_D*nbpts_linelets, 0, val)
            fastc.initNumaTransfer(uline[1]          , param_real, ptuline, Nbpts_D*nbpts_linelets, 0, val)
            fastc.initNumaTransfer(nutildeline[1]    , param_real, ptnutildeline, Nbpts_D*nbpts_linelets, 0, val)
            fastc.initNumaTransfer(psiline[1]        , param_real, ptpsiline, Nbpts_D*nbpts_linelets, 0, val)
            fastc.initNumaTransfer(matmline[1]       , param_real, ptmatmline, Nbpts_D*nbpts_linelets, 0, val)
            fastc.initNumaTransfer(matline[1]        , param_real, ptmatline, Nbpts_D*nbpts_linelets, 0, val)
            fastc.initNumaTransfer(matpline[1]       , param_real, ptmatpline, Nbpts_D*nbpts_linelets, 0, val)
            fastc.initNumaTransfer(alphasbetaline[1] , param_real, ptalphasbetaline, Nbpts_D, 0, val)
            fastc.initNumaTransfer(indexline[1]      , param_real, ptindexline, Nbpts_D, 0, val)

        if model=='LBMLaminar':
            if qloc_1[0] is not None:
                for f_i in range (0,neq_loc):
                    fastc.initNumaTransfer(qloc_1[f_i][1] , param_real, ptqloc_1[f_i] , Nbpts_D , 0, val)
            if qloc_2[0] is not None:
                for f_i in range (0,neq_loc):
                    fastc.initNumaTransfer(qloc_2[f_i][1] , param_real, ptqloc_2[f_i] , Nbpts_D , 0, val)
            if qloc_3[0] is not None:
                for f_i in range (0,neq_loc):
                    fastc.initNumaTransfer(qloc_3[f_i][1] , param_real, ptqloc_3[f_i] , Nbpts_D , 0, val)
    # recopie  InterpD
    fastc.initNumaTransfer(InterpD[1] , param_real, pt_coef , Nbpts_InterpD , 0, val)
...
# graphloc: list des processus sources pour le processus rank
# kpos: nombre processus source qui envoient a rank
#
def _procSource(rank, S_pos, pos_list, graph, graphloc, graphrcv_, filterGraph=None):

    pos_list.append(S_pos)
    k_pos= 0
    for proc in graph.keys(): # proc envoyeur
        #print('proc in graphKey', proc)
        for n in graph[proc].keys(): # n = proc recepteur
            if n == rank:     #rank = No process mpirun
