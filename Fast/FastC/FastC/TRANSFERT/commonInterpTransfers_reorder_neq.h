...
// #endif
     for (E_Int noind = pt_deb; noind < pt_fin; noind++)
     {
      ...
      val =0;
      if(K_FUNC::E_abs( ptrCoefs[ indCoef   ]) > cutoff_coef) val += ptrCoefs[ indCoef     ]*vectOfDnrFields[ne][ind000];
      if(K_FUNC::E_abs( ptrCoefs[ indCoef+1 ]) > cutoff_coef) val += ptrCoefs[ indCoef + 1 ]*vectOfDnrFields[ne][ind100];
      if(K_FUNC::E_abs( ptrCoefs[ indCoef+2 ]) > cutoff_coef) val += ptrCoefs[ indCoef + 2 ]*vectOfDnrFields[ne][ind010];
      if(K_FUNC::E_abs( ptrCoefs[ indCoef+3 ]) > cutoff_coef) val += ptrCoefs[ indCoef + 3 ]*vectOfDnrFields[ne][ind110];
      if(K_FUNC::E_abs( ptrCoefs[ indCoef+4 ]) > cutoff_coef) val += ptrCoefs[ indCoef + 4 ]*vectOfDnrFields[ne][ind001];
      if(K_FUNC::E_abs( ptrCoefs[ indCoef+5 ]) > cutoff_coef) val += ptrCoefs[ indCoef + 5 ]*vectOfDnrFields[ne][ind101];
      if(K_FUNC::E_abs( ptrCoefs[ indCoef+6 ]) > cutoff_coef) val += ptrCoefs[ indCoef + 6 ]*vectOfDnrFields[ne][ind011];
      if(K_FUNC::E_abs( ptrCoefs[ indCoef+7 ]) > cutoff_coef) val += ptrCoefs[ indCoef + 7 ]*vectOfDnrFields[ne][ind111];
      ...
      for (E_Int kk=0; kk<4; kk++)
        for (E_Int jj=0; jj<4; jj++)
          for (E_Int ii=0; ii<4; ii++)
          {
            ...
            if(K_FUNC::E_abs(ampli) > cutoff_coef) { val += ampli*vectOfDnrFields[ne][indD];}
          }
      vectOfRcvFields[ne][indR] = val;
